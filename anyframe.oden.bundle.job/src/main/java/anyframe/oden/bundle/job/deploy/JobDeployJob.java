...
import anyframe.oden.bundle.job.RepoFile;
import anyframe.oden.bundle.job.RepoManager;
...
public class JobDeployJob extends DeployJob {
	...
	public JobDeployJob(BundleContext context, CfgSource source, 
	...
	private Map<RepoFile, Collection<DeployFile>> groupByPath(Collection<DeployFile> files){
			...
			for(DeployFile f : sameFiles){				
				...
				try{
					...
					if(f.mode() == Mode.DELETE){
						...
						f.setDate(t);
						DeployerHelper.readyToDeploy(ds, f, useTmp);

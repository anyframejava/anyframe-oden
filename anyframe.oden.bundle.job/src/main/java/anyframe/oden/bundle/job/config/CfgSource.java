...
public class CfgSource {
	...
	public CfgSource(String path, String excludes, List<CfgMapping> mappings){
		this(path, excludes == null ? Collections.EMPTY_LIST : 
			Arrays.asList(excludes.split(",(\\s)*")), mappings);
	}
	
	public CfgSource(String path, List<String> excludes, List<CfgMapping> mappings){
		this.path = path;

...
public class SnapshotView extends ViewPart {
	...
	public static final String PLUGIN_ID = "anyframe.oden.eclipse.core.views.SnapshotView"; //$NON-NLS-1$

	private static int agentIndex;
	private static Composite compositeWhole;
	private static ArrayList<Agent> agentList;
	private static Combo agentCombo;
	...
	public void createPartControl(Composite parent) {
		...
		// Right-Top Composite(Agent)
		GridLayout layoutLeft = new GridLayout();
		Composite compositeAgent = new Composite(compositeWhole, SWT.NONE);
		compositeAgent.setLayout(layoutLeft);
		GridData data2 = new GridData(GridData.FILL_HORIZONTAL);
		compositeAgent.setLayoutData(data2);

		selectOdenAgent(compositeAgent);

		// separator for agent and detail
		Label separator1 = new Label(compositeWhole, SWT.SEPARATOR
				| SWT.HORIZONTAL);
		separator1.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		// Right-Bottom Composite(Detail)
		compositeDetail = new Composite(compositeWhole, SWT.NONE | SWT.V_SCROLL);
	...
	private void addContextMenu() {
		...
		MenuManager menuManager = new MenuManager(
				OdenMessages.ODEN_SNAPSHOT_SnapshotView_ContextManager);
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				JSONArray snapshotFileList = SnapshotViewContentProvider
						.getSnaphotFileList(
								SHELL_URL,
								OdenMessages.ODEN_SNAPSHOT_SnapshotView_MsgInfoFile
										+ " -json"); //$NON-NLS-1$
	...
	private void displayPlan(final Composite composite, String plan) {
		...
		try {
			...
			OdenActivator.error(OdenMessages.ODEN_SNAPSHOT_SnapshotView_Exception_GetSnapshotPlanInfo, e);
		}
		...
		try {
			...
			OdenActivator.error(OdenMessages.ODEN_SNAPSHOT_SnapshotView_Exception_ParseSnapshotPlanInfo, e);
		}

		Cmd cmd = new Cmd("\""+plan+"\"" + " = " + planInfo); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

		String tar = cmd.getOptionArg(TARGET_OPT);
		...
		group.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_PlanGroupTitle);
		...
		label.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelPlanName);
		...
		userIP.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelUserInfo);
		...
		enrollDate
				.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelPlanDate);
		...
		agent.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelDestAgent);
		...
		Label target = new Label(group, SWT.SINGLE);
		target.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelTargetLocation);

		final Text targetLocText = new Text(group, SWT.SINGLE | SWT.BORDER);
		GridData gridData4 = new GridData(GridData.FILL_HORIZONTAL);
		if (tar.length() == 1 || tar == null) {
			tar = ""; //$NON-NLS-1$
		} else {
			tar = tar.substring(1);
		}
		targetLocText.setText(tar);
		targetLocText.setLayoutData(gridData4);

		Label labelDest = new Label(group, SWT.SINGLE);
		labelDest
				.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelDestination);
		...
		labelDesc
				.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelPlanDescription);
		...
		buttonSave
				.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonPlanSave);
		buttonSave
				.setToolTipText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonTooltipPlanSave);
		buttonSave.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				targetLocation = targetLocText.getText();
		...
		buttonDelete
				.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonPlanDelete);
		buttonDelete
				.setToolTipText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonTooltipPlanDelete);
		...
		ImageDescriptor planImageDescriptor = ImageUtil
				.getImageDescriptor(OdenMessages.ODEN_SNAPSHOT_SnapshotView_TakeSnapshotIcon);
		...
		buttonTakeSnapshot
				.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonTakeSnapshot);
		buttonTakeSnapshot
				.setToolTipText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonTooltipTakeSnapshot);
	...
	private void displayFile(Composite composite, final String string) {
		...
		try {
			fileInfo = SnapshotViewContentProvider.getInfo(SHELL_URL,
					OdenMessages.ODEN_SNAPSHOT_SnapshotView_MsgInfoFile + " " + string + " -json"); //$NON-NLS-1$ //$NON-NLS-2$
		} catch (OdenException e) {
			OdenActivator.error(OdenMessages.ODEN_SNAPSHOT_SnapshotView_Exception_GetSnapshotDetailInfo, e);
		}
		...
		Group group = new Group(compositeDetail, SWT.SHADOW_ETCHED_IN);
		group
				.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_SnapshotGroupTitle);
		...
		Label label = new Label(group, SWT.SINGLE);
		label.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelFileName);
		...
		source.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelFileSize);

		Text fileSize = new Text(group, SWT.SINGLE | SWT.READ_ONLY);
		GridData gridData2 = new GridData(GridData.FILL_HORIZONTAL);
		fileSize.setText(fileSizeComma
				+ OdenMessages.ODEN_SNAPSHOT_SnapshotView_FileSizeTail);
		fileSize.setLayoutData(gridData2);

		Label date = new Label(group, SWT.SINGLE);
		date.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_LabelFileDate);
		...
		Button buttonDelete = new Button(compositeButton, SWT.PUSH);
		buttonDelete
				.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonSnapshotDelete);
		buttonDelete
				.setToolTipText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonTooltipSnapshotDelete);
		buttonDelete.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
		...
		ImageDescriptor rollbackImageDescriptor = ImageUtil
				.getImageDescriptor(OdenMessages.ODEN_SNAPSHOT_SnapshotView_RollbackIcon);
		Image rollbackImage = ImageUtil.getImage(rollbackImageDescriptor);

		Button buttonRollback = new Button(compositeButton, SWT.PUSH);
		buttonRollback
				.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonRollback);
		buttonRollback
				.setToolTipText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_ButtonTooltipRollback);
	...
	private void selectOdenAgent(Composite parent) {
		selectedName = null;
		...
		GridData gridData1 = new GridData(GridData.FILL_HORIZONTAL);
		label.setText(OdenMessages.ODEN_SNAPSHOT_SnapshotView_AgentSelect);
		label.setLayoutData(gridData1);

		agentCombo = new Combo(compositeCombo, SWT.READ_ONLY);
		GridData gridData2 = new GridData(GridData.FILL_HORIZONTAL);
		agentCombo.setLayoutData(gridData2);
	...
	private static void listenCombo() {
		selectedName = agentCombo.getText();
		agentIndex = agentCombo.getSelectionIndex();
		SnapshotNewPlanDialog.agent = selectedName;
		NewSnapshotPlanAction.agent = selectedName;

		int n = agentList.size();

		for (int i = 0; i < n; i++) {
			if (selectedName.equals(agentList.get(i).getNickname())) {
				showAgentList(agentList.get(i));
			}
		}
	}

	private static void setAgentList() {

		AgentManager manager = OdenActivator.getDefault().getAliasManager()
				.getAgentManager();
		Collection<Agent> col = manager.getAgents();
		Object[] list = col.toArray();
		agentList = new ArrayList<Agent>();

		for (int i = 0; i < list.length; i++) {
			agentList.add((Agent) list[i]);
			String nickName = agentList.get(i).getNickname();
			agentCombo.add(nickName);
		}
		if (selectedName == null) {
			agentCombo.select(0);
		} else {
			agentCombo.select(agentIndex);
		}
	}

	/**
	 * Refresh agent combo box.
	 */
	...
	private static void showAgentList(Agent agent) {
		SHELL_URL = "http://" + agent.getUrl() + "/shell"; //$NON-NLS-1$ //$NON-NLS-2$
		 if (!checkConnect()) {
		 SHELL_URL = null;
		 }
		invisibleRoot = null;
	...
	private static Boolean checkConnect() {
		...
		try {
			...
			OdenActivator.error(OdenMessages.ODEN_SNAPSHOT_SnapshotView_Exception_CheckConnection, e);
		}
		if (result == null) {
//			DialogUtil.openMessageDialog(
//			OdenMessages.ODEN_SNAPSHOT_SnapshotView_MsgOdenConnectionTitle,
//			OdenMessages.ODEN_SNAPSHOT_SnapshotView_MsgOdenConnection,
//			MessageDialog.WARNING);
//			OdenActivator.warning("Oden Snapshot view: " + OdenMessages.ODEN_CommonMessages_UnableToConnectServer);
			return false;
		} else { // connect
			return true;
		}

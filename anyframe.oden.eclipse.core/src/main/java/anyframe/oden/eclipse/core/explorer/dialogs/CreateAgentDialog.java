...
/**
 * Creates, changes, and duplicates an Agent profile.
 * 
 * @author RHIE Jihwan
 * @version 1.0.0
 * @since 1.0.0 M2
 *
 */
public class CreateAgentDialog extends TitleAreaDialog {
	...
	private String titleCreate = OdenMessages.ODEN_EXPLORER_Dialogs_CreateAgentDialog_AddTitle;
	private String subtitleCreate = OdenMessages.ODEN_EXPLORER_Dialogs_CreateAgentDialog_AddSubtitle;

	private String titleChange = OdenMessages.ODEN_EXPLORER_Dialogs_CreateAgentDialog_EditTitle;
	private String subtitleChange = OdenMessages.ODEN_EXPLORER_Dialogs_CreateAgentDialog_EditSubtitle;

	private String titleCopy = OdenMessages.ODEN_EXPLORER_Dialogs_CreateAgentDialog_DuplicateTitle;
	private String subtitleCopy = OdenMessages.ODEN_EXPLORER_Dialogs_CreateAgentDialog_DuplicateSubtitle;
	...
	public CreateAgentDialog(Shell parentShell, Type type, Agent agent) {
		super(parentShell);
		this.agent = agent;
		this.type = type;
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (type != Type.CREATE) {
			nicknameField.setText(agent.getNickname());
			// If (type == Type.CREATE) then it will use sequential nickname such as "new-agent-[agent serial number]" internally.
		...
		nicknameField.addKeyListener(new KeyListener() {
			public void keyPressed(KeyEvent keyEvent) {
				CreateAgentDialog.this.validate();
			};
		...
		noUsernameRequired.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(final SelectionEvent e) {
				...
				if (checked) {
					...
					CreateAgentDialog.this.validate(); // validate the user field even if the check box is checked with "anonymous" text
				}
			}
		});
		if (agent.isHasNoUserName()) {
		...
		if (type != Type.CREATE) {
			if (agent.getUrl() != null) {
				urlField.setText(agent.getUrl());
			}
			if (agent.getUser() != null) {
				userField.setText(agent.getUser());
			}
	...
	protected void okPressed() {
		agent.setNickname(nicknameField.getText().trim());
		agent.setUrl(urlField.getText().trim());
		if(noUsernameRequired.getSelection()) {
			agent.setHasNoUserName(true);
		} else {
			agent.setHasNoUserName(false);
			if (userField.getText().trim().length() > 0) {
				agent.setUser(userField.getText().trim());
				agent.setPassword(passwordField.getText().trim());
			}
		}

		if (type != Type.CHANGE) {
			OdenActivator.getDefault().getAliasManager().getAgentManager().addAgent(agent);
		}

		agent.setNickname(this.nicknameField.getText().trim());

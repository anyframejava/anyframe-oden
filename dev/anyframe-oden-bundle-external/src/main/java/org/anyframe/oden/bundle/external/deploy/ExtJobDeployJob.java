...
public class ExtJobDeployJob extends DeployJob {
	...
	public ExtJobDeployJob(BundleContext context, List<CfgFileInfo> fileInfo, 
		...
		jobLogger = (JobLogService) BundleUtil.getService(context, JobLogService.class);
		Assert.check(jobLogger != null, "No proper Service: " + jobLogger.getClass().getName());
	...
	String getBackupLocation(BundleContext ctx){
		...
		if(!"true".equals(undo)) return "snapshot";
...
//	}
	...
	private RepoManager getRepoManager(List<CfgFileInfo> fileInfo) throws OdenException{
	...
	private Map<ExtRepoFile, Collection<DeployFile>> groupByPath(Collection<DeployFile> files){
	...
	protected void deploy(Collection<DeployFile> repofiles) throws OdenException {
		...
		for(ExtRepoFile rf : fs.keySet()) {
			...
			// init deployer
			Map<DeployFile, DeployerService> inProgressFiles 
					= Collections.synchronizedMap(new HashMap<DeployFile, DeployerService>());
			...
			for(DeployFile f : sameFiles){				
				...
				try{
					...
					if(ds == null)
						throw new OdenException("Invalid agent: " + f.getAgent().agentAddr()+port);
...
//		for(AgentLoc t : otherTargets(_targets)){
		for(AgentLoc t : _targets){
			DeployerService ds = deployerManager.getDeployer(t.agentAddr() + port);
			if(ds == null) continue;
	...
	protected DeployFile getSameTargetFile(Collection<DeployFile> fs, CfgTarget tg){
				...
				public void run() {
					try {
						DoneFileInfo info = DeployerHelper.close(deployer, f,
								null, backupLocation.equals("snapshot")? null:FileUtil.combinePath(backupLocation,id));

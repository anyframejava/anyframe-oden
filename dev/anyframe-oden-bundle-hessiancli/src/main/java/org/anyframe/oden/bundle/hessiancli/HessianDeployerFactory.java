...
/**
 * This is HessianDeployerFactory class.
 * 
...
public class HessianDeployerFactory implements DeployerFactory {
	...
	protected Map<String, DeployerService> deploys = new ConcurrentHashMap<String, DeployerService>();
	...
	public DeployerService newInstance(String addr) {
		try {
			...
			Thread.currentThread().setContextClassLoader(
					DeployerService.class.getClassLoader());
			return ds;
		} catch (Exception e) {
			Logger.error(e);
		}
	...
	protected synchronized DeployerService createNewDeployer(String addr)
		...
		return (DeployerService) proxyfactory.create(
				Class.forName(DeployerService.class.getName()), "http://"
						+ addr + "/deploy", this.getClass().getClassLoader());
	}

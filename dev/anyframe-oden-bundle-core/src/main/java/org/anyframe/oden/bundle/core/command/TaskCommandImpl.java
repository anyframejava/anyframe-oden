...
public class TaskCommandImpl extends OdenCommand {
	...
	public void execute(String line, PrintStream out, PrintStream err) {
		...
		try {
			...
			if (Cmd.INFO_ACTION.equals(action)) {
				...
				if (isJSON) {
					...
					for (DeployFile dfile : dfiles) {
						buf.append(DeployFileUtil.modeToString(dfile.mode())
								...
								+ (!StringUtil.empty(dfile.errorLog()) ? " ["
										+ dfile.errorLog() + "]\n" : "\n"));
					}
				...
				throw new OdenException("Couldn't execute that action: "
						+ action);
			}
	...
	private String deploy(final String taskName, boolean isSync,
			final String user) throws OdenException {

...
public class DeployLogImpl implements DeployLogService {
	...
	private File getHistoryFile(long date) {
		...
		final String fname = LOG_FILE_PREFIX + onlyDateForFile(date)
				+ LOG_FILE_EXT;
	 ...
	 * @deprecated
	 */
	public List<RecordElement2> search(String txid, String user, String agent,
			String path, String startdate, String enddate, boolean failonly)
		...
		try {
			if (!empty(txid)) { // retrieve data by transaction id
				...
				long start = empty(startdate) ? Long.MIN_VALUE
						: longTime(startdate + START_TIME);
				long end = empty(enddate) ? Long.MAX_VALUE : longTime(enddate
						+ END_TIME);
	...
	private RecordElement2 latestRecord(File f, boolean failonly)
			throws OdenException {
		Object o = null;
	...
	private List<RecordElement2> getRecords(long startdate, long enddate)
			throws OdenException {
		...
		for (File logfile : logfiles) {
			if (acceptedFile(logfile, startdate, enddate)) {
				if (latest != null
						&& logfile.getName().equals(latest.getName())) {
					synchronized (latch) {
	...
	private List<MiniRecordElement> getMiniRecords(long startdate, long enddate)
			throws OdenException {
		List<MiniRecordElement> records = new ArrayList<MiniRecordElement>();
		File[] logfiles = logfiles();
		File latest = latestFile(logfiles);
		for (File logfile : logfiles) {
			if (acceptedFile(logfile, startdate, enddate)) {
				if (latest != null
						&& logfile.getName().equals(latest.getName())) {
					synchronized (latch) {
						collectMiniRecords(logfile, records);
					}
				} else {
					collectMiniRecords(logfile, records);
				}
	...
	private void collectRecords(File logfile, List<RecordElement2> records)
			throws OdenException {
		FileObjectInputStream fin = null;
		try {
			synchronized (latch) {
			...
			throw new OdenException("Illegal format: "
	...
	private void collectMiniRecords(File logfile,
		...
		try {
			synchronized (latch) {
				...
				while (fin.available() > 0) {
					RecordElement2 r = (RecordElement2) fin.readObject();
					records.add(new MiniRecordElement(r.id(), r
							.getDeployFiles().size(), r.getUser(), r.getDate(),
							r.isSuccess(), r.log(), r.desc()));
				}
			}
		} catch (ClassNotFoundException e) {
			throw new OdenException("Illegal format: "
	...
	private long getDateFromFile(String fname) throws ParseException {
		String sdate = fname.substring(LOG_FILE_PREFIX.length(), fname.length()
				- LOG_FILE_EXT.length());
		return longDate(sdate);
	}

	private List<RecordElement2> searchByUser(String user,
	...
	private List<MiniRecordElement> searchMiniByUser(String user,
	...
	private List<RecordElement2> searchByAgent(String agent,
			List<RecordElement2> records) {
		List<RecordElement2> result = new ArrayList<RecordElement2>();
		for (RecordElement2 record : records) {
	...
	private List<RecordElement2> searchByPath(String fname,
			List<RecordElement2> records) {
		List<RecordElement2> result = new ArrayList<RecordElement2>();
		for (RecordElement2 record : records) {
	...
	private List<MiniRecordElement> refineFailRecordonly(
			List<MiniRecordElement> records) {
		List<MiniRecordElement> result = new ArrayList<MiniRecordElement>();
		for (MiniRecordElement record : records) {
			if (!record.isSuccess()) {

...
/**
 * This is ServicesCommandImpl class.
 * 
		...
		if (tokens.size() >= 1) {
			while (tokens.size() > 0) {
				...
				try {
					...
					if (bundle != null) {
						...
						for (int refIdx = 0; (refs != null)
								&& (refIdx < refs.length); refIdx++) {
							String[] objectClass = (String[]) refs[refIdx].getProperty("objectClass");

							// Determine if we need to print the service,
							// depending
							// on whether it is a command service or not.
							boolean print = true;
							for (int ocIdx = 0; !showAll
									&& (ocIdx < objectClass.length); ocIdx++) {
								if (objectClass[ocIdx]
										.equals(org.apache.felix.shell.Command.class
												.getName())) {
									print = false;
								}
							}
							...
							if (showAll || print) {
								...
								for (int keyIdx = 0; (keys != null)
										&& (keyIdx < keys.length); keyIdx++) {
			...
			if (bundles != null) {
				...
				for (int bundleIdx = 0; bundleIdx < bundles.length; bundleIdx++) {
					...
					for (int refIdx = 0; (refs != null)
							&& (refIdx < refs.length); refIdx++) {
						String[] objectClass = (String[]) refs[refIdx]
						...
						if (showAll || print) {
							if (!headerPrinted) {
								headerPrinted = true;
								String title = Util
										.getBundleName(bundles[bundleIdx]);
								title = (inUse) ? title + " uses:" : title
										+ " provides:";
								out.println("\n" + title);
								out.println(Util.getUnderlineString(title));

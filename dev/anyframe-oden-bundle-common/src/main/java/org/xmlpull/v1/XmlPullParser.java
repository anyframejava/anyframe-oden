 ...
 * <a href="http://www.xmlpull.org/">XMLPULL V1 API</a> (visit this website to
 ...
 * is intened <b>for operation in constrained environments</b> such as J2ME.
 * </ul>
 * 
 * 
 * <p>
 ...
 * 
 * <p>
 * The current event state of the parser can be determined by calling the <a
 * href="#getEventType()">getEventType()</a> method. Initially, the parser is in
 * the <a href="#START_DOCUMENT">START_DOCUMENT</a> state.
 * 
 * <p>
 ...
 * <dd>Text content was read; the text content can be retreived using the
 ...
 * <ul>
 * <li><b>version</b>: getProperty(&quot;<a
 * href="http://xmlpull.org/v1/doc/properties.html#xmldecl-version"
 * >http://xmlpull.org/v1/doc/properties.html#xmldecl-version</a>&quot;) returns
 * String ("1.0") or null if XMLDecl was not read or if property is not
 * supported
 * <li><b>standalone</b>: getProperty(&quot;<a
 * href="http://xmlpull.org/v1/doc/features.html#xmldecl-standalone"
 ...
 * is not supported otherwise returns Boolean(true) if standalon="yes" and
...
public interface XmlPullParser {
	 ...
	 * getEvent() before the first call to next(), nextToken, or nextTag()</a>).
	 * 
	 * @see #next
	 * @see #nextToken
	 */
	 ...
	 * <p>
	 * <strong>NOTE:</strong> calling again <a href="#next()">next()</a> or <a
	 * href="#nextToken()">nextToken()</a> will result in exception being
	 * thrown.
	 * 
	 * @see #next
	 * @see #nextToken
	...
	/**
	 * Returned from getEventType(), <a href="#next()">next()</a>, <a
	 * href="#nextToken()">nextToken()</a> when a start tag was read. The name
	 * of start tag is available from getName(), its namespace and prefix are
	 * available from getNamespace() and getPrefix() if <a
	 * href='#FEATURE_PROCESS_NAMESPACES'>namespaces are enabled</a>. See
	...
	/**
	 * Returned from getEventType(), <a href="#next()">next()</a>, or <a
	 * href="#nextToken()">nextToken()</a> when an end tag was read. The name of
	 * start tag is available from getName(), its namespace and prefix are
	 ...
	 * exact roundtrip without chnanging line ends when examining low level
	 ...
	 * contained in the CDATA section is available by callling getText().
	 * 
	 * @see #nextToken
	 * @see #getText
	 */
	...
	/**
	 * An entity reference was just read; this token is available from <a
	 * href="#nextToken()">nextToken()</a> only. The entity name is available by
	 * calling getName(). If available, the replacement text can be obtained by
	 * calling getTextt(); otherwise, the user is responsibile for resolving the
	 * entity reference. This event type is never returned from next(); next()
	 * will accumulate the replacement text and other text events to a single
	 * TEXT event.
	 * 
	 * @see #nextToken
	 * @see #getText
	...
	/**
	 * Ignorable whitespace was just read. This token is available only from <a
	 * href="#nextToken()">nextToken()</a>). For non-validating parsers, this
	 ...
	 * contents of the array may be dangerous since malicous applications may
	 * alter the array, although it is final, due to limitations of the Java
	 * language.
	 */
	String[] TYPES = { "START_DOCUMENT", "END_DOCUMENT", "START_TAG",
			"END_TAG", "TEXT", "CDSECT", "ENTITY_REF", "IGNORABLE_WHITESPACE",
	 ...
	 * 1.0 sepcification are reported. This implies that FEATURE_PROCESS_DOCDECL
	 ...
	 * requested, all feautures are deactivated by default.
	 * 
	 ...
	 * href="http://xmlpull.org/v1/doc/features.html#detect-encoding"
	 * >http://xmlpull.org/v1/doc/features.html#detect-encoding</a> MUST be true
	 ...
	 * Otherwise, if inputEncoding is null and the parser suppports the encoding
	 * detection feature
	...
	/**
	 * Set new value for entity replacement text as defined in <a
	 * href="http://www.w3.org/TR/REC-xml#intern-replacement">XML 1.0 Section
	...
	/**
	 * Returns the namespace prefixe for the given position in the namespace
	 ...
	 * efifcient lookup, e.g. using a Hashtable. The 'xml' prefix is bound to
	 * "http://www.w3.org/XML/1998/namespace", as defined in the <a
	 * href="http://www.w3.org/TR/REC-xml-names/#ns-using">Namespaces in XML</a>
	 * specification. Analogous, the 'xmlns' prefix is resolved to <a
	 ...
	 * namespace attributes (xmlns:ns='...') must be reported with namespace <a
	 * href="http://www.w3.org/2000/xmlns/">http://www.w3.org/2000/xmlns/</a>
	 ...
	 * 
	 * @param zero
	 *            based index of attribute
	 * @return attribute namespace, empty string ("") is returned if namesapces
	 ...
	 * entity replacement text if PROCESS_DOCDECL is false) as described in <a
	 ...
	 * processing instructions will be ignored and emtity references must be
	 * expanded or exception mus be thrown if entity reerence can not be
	 * exapnded). If element content is empty (content is "") then no TEXT event
	 * will be reported.
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * If special feature <a
	 * href="http://xmlpull.org/v1/doc/features.html#xml-roundtrip"
	 * >FEATURE_XML_ROUNDTRIP</a> (identified by URI:
	 ...
	 * normalized as described <a
	 * href="http://www.w3.org/TR/REC-xml#sec-line-ends">XML 1.0 End-of-Line
	 ...
	 * element will always reported as IGNORABLE_WHITESPACE otherise rteporting
	 * is optional. <br>
	 * Note: that element content may be delevered in multiple consecutive
	 * IGNORABLE_WHITESPACE events.
	 * <dt>CDSECT
	 * <dd>
	 * return text <em>inside</em> CDATA (ex. 'fo&lt;o' from
	 * &lt;!CDATA[fo&lt;o]]>)
	 ...
	 * of PI target, space and data so for example for &lt;?target data?> string
	 * &quot;target data&quot; may be returned if FEATURE_XML_ROUNDTRIP is
	 * false.
	 * <dt>COMMENT
	 ...
	 * <dt>DOCDECL
	 * <dd>
	 * if FEATURE_XML_ROUNDTRIP is true or PROCESS_DOCDECL is false then return
	 ...
	 * 
	 * <p>
	 * <strong>NOTE:</strong> there is no gurantee that there will only one TEXT
	 * or IGNORABLE_WHITESPACE event from nextToken() as parser may chose to
	 * deliver element content in multiple tokens (dividing element content into
	 * chunks)
	 * 
	 * <p>
	 ...
	 * 		throw new XmlPullParserException(
	 * 				&quot;event TEXT it must be immediately followed by END_TAG&quot;, this,
	 * 				null);
	 * 	}
	 * 	return result;

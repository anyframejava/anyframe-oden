...
public class BDBJobLogger implements JobLogService {
	...
	private long getDateFromFile(String fname) throws ParseException {
		String sdate = fname.substring(LOG_FILE_PREFIX.length(), fname.length()
				- LOG_FILE_EXT.length());
		return new SimpleDateFormat(FILE_NAME_DATE_PATTERN).parse(sdate)
				.getTime();
	}
	...
	public List<ShortenRecord> search(String job, String user, String path,
		...
		synchronized (latch) {
			try {
				...
				if (infoCursor.getLast(key, data, LockMode.DEFAULT) != OperationStatus.SUCCESS)
					return ret;
				do {
					...
					if (!StringUtil.empty(path)
							&& show(conn, found.getId(), path, Mode.NA,
									isFailOnly).size() == 0)
						continue;
					ret.add(found);
				} while (infoCursor.getPrev(key, data, LockMode.DEFAULT) == OperationStatus.SUCCESS);
				return ret;
			} catch (Exception e) {
				throw new OdenException(e);
			} finally {
		...
		synchronized (latch) {
			try {
				...
				if (StringUtil.empty(id)) {
					...
					if (cur.getLast(new DatabaseEntry(), data, LockMode.DEFAULT) != OperationStatus.SUCCESS)
						throw new OdenException("Fail to get: " + INFO_DB);
				} else {
					if (infoDb.get(null,
							new DatabaseEntry(id.getBytes("utf-8")), data,
	...
	private Set<DeployFile> show(BDBConnection conn, String id, String path,
			Mode mode, boolean isFailOnly) throws OdenException {
	...
	public void record(RecordElement2 record) throws OdenException {
		...
		synchronized (latch) {
			try {
				...
				DatabaseEntry key = new DatabaseEntry(record.id().getBytes(
						"utf-8"));
				...
				DeployFileListBinding deployFileListBinding = new DeployFileListBinding();
				deployFileListBinding.objectToEntry(record.getDeployFiles(),
						filesData);
	...
	public void record(String id, String user, long time, String desc,
		...
		synchronized (latch) {
			try {
				...
				DatabaseEntry infoData = new DatabaseEntry();
				RecordInfoBinding recordInfoBinding = new RecordInfoBinding();
				recordInfoBinding.objectToEntry(new RecordElement2(id,
						new FooCollection<DeployFile>(fs.size()), nSuccess,
						user, time, error, desc), infoData);
				infoDb.put(tx, key, infoData);
	...
	private void removeOld(Transaction tx, Database infoDb,
			RecordInfoBinding infoBind, Database filesDb) throws OdenException {
		...
		try {
			...
			DatabaseEntry key = new DatabaseEntry();
			DatabaseEntry data = new DatabaseEntry();
			while (cur.getFirst(key, data, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
				Object o = infoBind.entryToObject(data);
				if (o == null)
					continue;
				ShortenRecord found = (ShortenRecord) o;

				if (found.getDate() > minLimit)
					break;
				cur.delete();
	...
	private long getMinLimitDate() {
		...
		GregorianCalendar cal = new GregorianCalendar(_cal.get(Calendar.YEAR),
				_cal.get(Calendar.MONTH), _cal.get(Calendar.DAY_OF_MONTH));
		try {

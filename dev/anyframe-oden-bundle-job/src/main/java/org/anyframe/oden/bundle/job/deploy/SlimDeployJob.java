...
public class SlimDeployJob extends Job {
	...
	public SlimDeployJob(BundleContext context, CfgSource source, String user,
			String desc, List<CfgTarget> targets,
		...
		// repository service
		RepositoryProviderService repoProvider = (RepositoryProviderService) BundleUtil
				.getService(context, RepositoryProviderService.class);
		Assert.check(repoProvider != null, "No proper Service: "
	...
	protected void deploy() throws OdenException {
		...
		SlimDeployManager deployMgr = new SlimDeployManager(context, id,
				"true".equals(undo));
		...
		for (RepoFile rf : fs.keySet()) {
			...
			// init deployer
			Map<SlimDeployFile, DeployerService> inProgressFiles = Collections
					.synchronizedMap(new HashMap<SlimDeployFile, DeployerService>());
			...
			for (SlimDeployFile f : sameFiles) {
				...
				try {
					DeployerService ds = deployMgr.getDeployer(targetCfgMap
							.get(f.getTarget()).getAddress());
					if (ds == null)
						throw new OdenException("Invalid target: "
								+ f.getTarget());

					if (f.getMode() == Mode.DELETE) {
						ds.backupNRemove(targetCfgMap.get(f.getTarget())
								.getPath(), f.getPath(), backupLocation,
								backupcnt);
						f.setSuccess(true);
						nSuccess++;
					} else { // add or update
						try {
							ds.init(targetCfgMap.get(f.getTarget()).getPath(),
									f.getPath(), t, useTmp, 0,false);
						} catch (Exception e) {
							try {
								ds.close(targetCfgMap.get(f.getTarget())
										.getPath(), f.getPath(), null, null);
							} catch (Exception ee) {
							}
							throw e;
						}
	...
	/**
	 * When deploying files with update mode, some files will not be deployed to
	 * some targets. Because those targets already have those files. In that
	 * case, those files' last modified date become to be different.
	 * 
	 * @param fmap
	 * @param date
	 */

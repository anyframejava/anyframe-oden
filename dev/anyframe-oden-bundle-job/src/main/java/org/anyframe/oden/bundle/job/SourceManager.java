...
public class SourceManager extends RepoManager {
	...
	public void getCandidatesFileInfo(Collection<FileInfo> ret)
		...
		repo.listAllFilesFileInfo(repoArgs, null, dirSrcMap, src.getExcludes(),
				ret);
	}
		...
		for (CfgMapping mapping : src.getMappings()) {
			if (mapping.getDir().equals("*")
				...
				if (!StringUtil.empty(mapping.getCheckoutDir())) {
					String converted = rf.getFile().endsWith(".class") ? getSrcFileName(rf
							.getFile()) : rf.getFile();
					String child = converted.substring(mapping.getDir()
							.length());
					String mappingFile = FileUtil.isAbsolutePath(mapping
							.getCheckoutDir()) ? FileUtil.combinePath(
							mapping.getCheckoutDir(), child) : FileUtil
							.resolveDotNatationPath(FileUtil.combinePath(
									src.getPath(), mapping.getCheckoutDir()))
							+ child;
					mappings.add(mappingFile);
				}
			}
		}
		return repo.resolveAsFileInfo(makeToRepoArgs(rf.getSubdir()), mappings,
				rf.getFile());
	}

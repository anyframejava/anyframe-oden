...
public class LogCommandImpl implements CustomCommand {
	...
	private String execute(Cmd cmd, boolean isJSON) throws Exception {
		...
		if (action.equals("search")) {
						...
						pageHandler.getCachedData(cmd, pgscale,
								new PageHandlerOr() {
									public JSONArray run() throws Exception {
										final List<ShortenRecord> ret = jobLogger
												.search(job, user, path,
														isFailOnly);

										JSONArray ja = new JSONArray();
										for (ShortenRecord r : ret) {
											String status = r.isSuccess() ? "S"
													: "F";
											ja.put(new JSONObject()
													.put("txid", r.getId())
													.put("date",
															DateUtil.toStringDate(r
																	.getDate()))
													.put("status", status)
													.put("job", r.getJob())
													.put("user", r.getUser())
			...
			List<ShortenRecord> ret = jobLogger.search(job, user, path,
					isFailOnly);
			...
			for (ShortenRecord r : ret) {
				...
				buf.append(r.getId() + "\t"
						+ DateUtil.toStringDate(r.getDate()) + "\t" + status
						+ " (" + r.getnSuccess() + "/" + r.getTotal() + ")\t"
						+ r.getJob() + "(" + r.getUser() + ")\n");
			}
			...
			JSONObject obj = pageHandler.getCachedData(cmd, pgscale,
					new PageHandlerOr() {
						public JSONArray run() throws Exception {
							return (JSONArray) toJSONArray2(new SortedDeployFileSet(
			...
			if (isJSON) {
				return new JSONArray()
						.put(new JSONObject()
								...
								.put("job", r.getJob()).put("log", r.getLog()))
						.toString();
			}

			StringBuffer buf = new StringBuffer();
			buf.append(r.getId() + " " + (r.isSuccess() ? "Success" : "Fail")
					+ " " + r.getJob() + "(" + r.getTotal() + ") "
					+ DateUtil.toStringDate(r.getDate()) + " " + r.getLog()
					+ "\n\n");
			...
			if (isJSON) {
				return new JSONArray().put(
						new JSONObject().put("contents", r.getContents()).put(
	...
	private JSONArray toJSONArray2(SortedDeployFileSet fs) throws JSONException {
		...
		for (Iterator<DeployFile> it = fs.iterator(); it.hasNext();) {
			...
			list.put(new JSONObject()
					...
					.put("targets",
							new JSONArray().put(current.getAgent().agentName()))

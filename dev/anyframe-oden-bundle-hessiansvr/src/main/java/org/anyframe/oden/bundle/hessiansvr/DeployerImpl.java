...
//	}
	...
	public void init(String parent, String path, 
			long date, boolean useTmp, int backupcnt) throws Exception{
		parent = toAbsPath(parent);
...
//	}
		...
		if(dir != null && dir.exists() && dir.isDirectory()) {
			...
			for(int i=0; i<files.length; i++) {
				if(files[i].isDirectory()) {
					...
					info.setUpdate(DeployerUtils.undoBackup(root, info.getPath(), bak , backupcnt));
	...
	public DoneFileInfo backupNCopy(String srcPath, String filePath, 
		...
		synchronized (lock) {
			...
			//backup
			if(bakPath != null ){ 
				srcExist = DeployerUtils.undoBackup(destPath, filePath, bakPath, backupcnt);
			}
	 ...
	 * @throws IOException 
	 */
	public DoneFileInfo backupNRemove(String srcPath, String filePath, String bakPath, int backupcnt) throws Exception {
		srcPath = toAbsPath(srcPath);
		bakPath = toAbsPath(bakPath);
		...
		synchronized (lock) {
			...
			srcExist = DeployerUtils.undoBackup(srcPath, filePath, bakPath , backupcnt);

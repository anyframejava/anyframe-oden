...
public class Compile {
	...
	public void execute(BuildInfo build) throws Exception {
		...
		List buildList = build.toBuildObject();	
		...
		for(int i = 0; i < buildList.size(); i++){
			...
			CfgBuildDetail detail = (CfgBuildDetail) buildList.get(i);//임시추가
			...
			javac.setEncoding(build.getEncoding());
			...
			if( !".".equals(build.getClassesRoot2()) && !".".equals(build.getLibsRoot2()) )  {				
				// class path add2
				String classesDir2 = build.getClassesRoot2();
				Path classesPath2 = classPath.createPath();
				classesPath2.setPath(classesDir2);
				classPath.add(classesPath2);
				
				FileSet fs2 = new FileSet();
				//fs.setDir(new File(libs));
				fs2.setDir(new File(build.getLibsRoot2()));
				fs2.setIncludes("*.jar");			
				classPath.addFileset(fs2);				
			}
			
			
			if( !".".equals(build.getClassesRoot3()) && !".".equals(build.getLibsRoot3()) ){				
				// class path add3
				String classesDir3 = build.getClassesRoot3();
				Path classesPath3 = classPath.createPath();
				classesPath3.setPath(classesDir3);
				classPath.add(classesPath3);
				
				FileSet fs3 = new FileSet();
				//fs.setDir(new File(libs));
				fs3.setDir(new File(build.getLibsRoot3()));
				fs3.setIncludes("*.jar");			
				classPath.addFileset(fs3);				
			}
	...
	public void webTransfer(BuildInfo build) throws Exception {
		...
		String webAppd = build.getWebAppd().equals(".") ? "" : "/"
				+ build.getWebAppd() + "/";
		String resRoot = build.getResRoot();
		...
		for(int i = 0; i < buildList.size(); i++){
			...
			CfgBuildDetail detail = (CfgBuildDetail) buildList.get(i);
			...
			if(!".".equals(resRoot)){
				...
				Copy resCopy = new Copy();
				
				resCopy.setProject(project);
				resCopy.init();

				resCopy.setTaskName("Copy Web Resource");

 ...
 *
 */
public class TaskDeployJob2 extends DeployJob {
	...
	public TaskDeployJob2(BundleContext context, String user, String desc,
		...
		for(String path : fs.keySet()) {
			...
			for(DeployFile f : sameNameFiles){
				if(f.mode() == Mode.NA){
					...
					DeployerService ds = deployerManager.getDeployer(f);
					f.setDate(t);
					if(ds != null && DeployerHelper.readyToDeploy(ds, f)){
		...
		try{
			...
			while((size = in.read(buf)) != -1){
				for(DeployFile f : sameNameFiles){	
					...
					try{
						if(deployer != null && !deployer.write(buf, size))

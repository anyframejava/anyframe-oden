...
public class DeployLogImpl implements DeployLogService {
	...
	private List<MiniRecordElement> getMiniRecords(long startdate, long enddate) throws OdenException {		
	...
	private void collectMiniRecords(File logfile, List<MiniRecordElement> records) throws OdenException {
		FileObjectInputStream fin = null;
		try{
			synchronized (latch) {
				fin = new FileObjectInputStream(logfile);
				while(fin.available() > 0){
					RecordElement2 r = (RecordElement2)fin.readObject();
					records.add(new MiniRecordElement(
							r.id(), r.getDeployFiles().size(), r.getUser(), 
							r.getDate(), r.isSuccess(), r.log(), r.desc()));
				}
			}
		}catch(ClassNotFoundException e){
			throw new OdenException("Illegal format: " + logfile.getAbsolutePath());
	...
	private List<MiniRecordElement> searchMiniByUser(String user, List<MiniRecordElement> records) {
		List<MiniRecordElement> result = new ArrayList<MiniRecordElement>();
	...
	private List<MiniRecordElement> refineFailRecordonly(List<MiniRecordElement> records) {
		List<MiniRecordElement> result = new ArrayList<MiniRecordElement>();
		for(MiniRecordElement record : records){
			if(!record.isSuccess())
				result.add(record);
